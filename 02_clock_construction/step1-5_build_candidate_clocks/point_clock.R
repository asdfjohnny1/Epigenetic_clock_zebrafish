# ==========================================================================
# PIPELINE STAGE: 02 / Clock construction - steps 1-5 (Point)
# ==========================================================================
# Builds candidate Point epigenetic clocks: data prep, probe filtering,
...
# Point Epigenetic Clock Construction Pipeline (Steps 1–6)
#
# This pipeline processes DNA methylation data across multiple organs to build point epigenetic clocks, that is based on single CPG sites. 
...
output_directory <- file.path(PROJECT_ROOT, "multitissue/making_clocks/R/results/point_clock_final_final")
...
# ======================
# STEP 1 - Make function to process methylation files 
# ======================


# Directories for input/output
output_dir <- paste0(output_directory,"/step2")
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

process_organ_folder_long <- function(organ_folder_path, organ_name, r_squared_value) {
  message("Processing STEP 2 - organ: ", organ_name, " | R²: ", r_squared_value)
  
  file.list <- list.files(path = organ_folder_path, pattern = ".tsv", full.names = TRUE)
  if (length(file.list) == 0) {
    warning("No files in ", organ_folder_path)
    return(NULL)
  }
  ...
  for (i in seq_along(file.list)) {
    ...
    save_path <- file.path(output_dir , organ_name, r_squared_value)
...
# ======================
# STEP 2 -Apply to All Organs and Thresholds
# ======================


thresholds <- c("r2_nofilter" = "nofilter", "r2_0.05" = "0.05", "r2_0.1" = "0.1", "r2_0.2" = "0.2", "r2_0.3" = "0.3")

for (r_squared_value in names(thresholds)) {
  for (i in seq_along(organ_dirs)) {
    try({
      process_organ_folder_long(organ_dirs[i], organ_names[i], r_squared_value)
    }, silent = TRUE)
  }
}

message("Step 2 complete — all long-format files saved by organ and threshold.")
...
# ================
# Directories for input/output
input_root <- file.path(output_directory, "step2")  # input .rds files from Step 2
output_dir <- file.path(output_directory, "step3")
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

# Loop through thresholds and organs
for (r_name in names(thresholds)) {
  threshold_val <- thresholds[[r_name]]
  organ_dirs <- list.dirs(input_root, recursive = FALSE)
  
  for (organ_dir in organ_dirs) {
    organ_name <- basename(organ_dir)
    organ_threshold_dir <- file.path(organ_dir, r_name)
    ...
    for (rds_file in rds_files) {
      ...
      message("Processing STEP 3 : ", organ_name, " | ", r_name, " | ", sample_name)
      ...
      # Compute R² and mean methylation per probe
      metrics_df <- df %>%
        group_by(Probe) %>%
        summarise(
          r_squared = tryCatch({
      ...
      # Save metrics
      metrics_out_dir <- file.path(output_dir, organ_name, r_name)
      ...
      # Step 3.1: Apply R² threshold if not "nofilter"
      if (threshold_val != "nofilter") {
        filtered_df <- metrics_df %>%
          filter(!is.na(r_squared) & r_squared > as.numeric(threshold_val))
...
# List all organs in step3
organ_dirs <- list.dirs(input_dir, recursive = FALSE, full.names = TRUE)

for (organ_dir in organ_dirs) {
  organ_name <- basename(organ_dir)
  
  # Define output directory for this organ
  output_organ_dir <- file.path(output_dir, organ_name)
  dir.create(output_organ_dir, recursive = TRUE, showWarnings = FALSE)
  
  # List all R² threshold folders for this organ
  r2_dirs <- list.dirs(organ_dir, recursive = FALSE, full.names = TRUE)
  
  for (r2_dir in r2_dirs) {
    r_squared_value <- basename(r2_dir)
    output_r2_dir <- file.path(output_organ_dir, r_squared_value)
    ...
    for (file in metric_files) {
      ...
      message("Processing STEP 4 : ", organ_name, " | ", r_squared_value, " | ", sample_id)
...
message("✅ Step 4 completed: Initialised modelling scaffolds and saved per file.")
...
message("Starting STEP 5 - extracting filtered CpGs per organ and R² threshold")

# List all organ folders in step4
organ_dirs <- list.dirs(input_dir, recursive = FALSE, full.names = TRUE)
...
for (organ_dir in organ_dirs) {
  ...
  for (r2_dir in r2_dirs) {
    ...
    # List all clock setup files in this folder (created by Step 4)
    ...
    for (file in clock_setup_files) {
      ...
      message("Processing STEP 5 : ", organ_name, " | ", r_squared_value, " | ", sample_id)
      ...
      # Save current_probes per file in step5 directory
      saveRDS(current_probes, file = file.path(output_r2_dir, paste0(sample_id, "_current_probes.rds")))
    }
  }
}

message("✅ Step 5 completed: filtered CpG probe lists saved per organ and R² threshold.")



#------------------------------------------------------
# Step 6: Epigenetic Clock Model Training, Validation, and Selection Across Organs and R² Thresholds
#------------------------------------------------------
# Parameters
num_runs <- 1000  # 
...
for (organ_dir in organ_dirs) {
  ...
  # Get R² threshold dirs inside organ
  r2_dirs <- list.dirs(organ_dir, recursive = FALSE, full.names = TRUE)
  
  for (r2_dir in r2_dirs) {
    r_squared_value <- basename(r2_dir)
    
    # Output dir for this organ & R2
    output_r2_dir <- file.path(output_dir, organ_name, r_squared_value)
    dir.create(output_r2_dir, recursive = TRUE, showWarnings = FALSE)
    
    # List all sample scaffold files
    ...
    for (scaffold_file in scaffold_files) {
      ...
      message("Processing STEP 6 : ", organ_name, " | ", r_squared_value, " | ", sample_id)
      
      
      # Load scaffold & methylation data
      file_entry <- readRDS(scaffold_file)
      file_entry$methylation_data <- fread(paste0(input_directory_base,"/",organ_name,"/",sample_id,".tsv"))
      ...
      tryCatch({
        for (i in 1:num_runs) {
          ...
          for (j in 1:num_runs) {
            ...
            # 6.2: Selecting probes
            if (!is.null(file_entry$filtered_metrics) && nrow(file_entry$filtered_metrics) > 1) {
              num_selected_cpgs <- sample(1:nrow(file_entry$filtered_metrics), 1)
              selected_cpgs <- file_entry$filtered_metrics[sample(nrow(file_entry$filtered_metrics), num_selected_cpgs), , drop = FALSE]
            ...
            if (!is.null(file_entry$methylation_data_filtered) &&
              ...
              md_filtered <- na.omit(md_filtered)
...
message(" Step 6 completed: models fitted and validated for all organs, thresholds, and samples.")