# ==========================================================================
# PIPELINE STAGE: 02 / Clock construction - step 6 (Tile, Lasso)
# ==========================================================================
...
#
## for testing : scaffold_file <- file.path(PROJECT_ROOT, "multitissue/making_clocks/R/results/tile_clock_final_final/step4/cpg_island_tile_clock/All_organs/r2_0.05/tileClock_1000bp_methylation_matrix_min10_clock_setup.rds")
...
output_directory_base <- file.path(PROJECT_ROOT, "multitissue/making_clocks/R/results/tile_clock_final_final/step6.array.new.new/Lasso")
...
scaffold_dir <- dirname(scaffold_file)          # .../r2_0.05
r2_dir_name  <- basename(scaffold_dir)          # r2_0.05

organ_dir    <- dirname(scaffold_dir)           # .../All_organs
organ_name   <- basename(organ_dir)             # All_organs

tile_dir     <- dirname(organ_dir)              # .../cpg_island_tile_clock
tile_name    <- basename(tile_dir)              # cpg_island_tile_clock


output_r2_dir <- file.path(output_directory_base,tile_name, organ_name, r2_dir_name)
...
process_scaffold <- function(scaffold_file, sample_data, output_r2_dir, input_directory_base) {
  ...
  # Expect methylation tsv at: <input_directory_base>/<organ_name>/<sample_id>.tsv
  # where organ_name was derived earlier
  methylation_path <- file.path(input_directory_base,tile_name, organ_name, paste0(sample_id, ".tsv"))
  ...
  tryCatch({
    for (i in 1:num_runs_outer) {
      tryCatch({
        ...
        for (j in 1:num_runs_inner) {
          tryCatch({
            ...
            if (!is.null(file_entry$methylation_data_filtered) &&
              ...
              canonical_idx <- grepl("^chr([0-9]+|X|Y|M):", rownames(md_filtered))
            ...
            if (ncol(file_entry$methylation_data_train.final) > 1) {
              ...
              ## looking at different nlambda's to see if that impacts anything  - higher supports greater granularity
        ...
        if (!is.null(final_val_predicted_ages) && !is.na(final_validation_r_squared)) {
          file_entry$all_outer_validation_metrics[[paste0("outer_", i)]] <- list(
            outer_iteration = i,
            val_r_squared   = final_validation_r_squared,
