# ==========================================================================
# PIPELINE STAGE: 02 / Clock construction - step 6 (Point, Lasso)
# ==========================================================================
...
output_directory_base <- file.path(PROJECT_ROOT, "multitissue/making_clocks/R/results_split_organ/point_clock/step6.array.new.new/Lasso")
      ...
      tryCatch({
        ...
        for (j in 1:num_runs_inner) {
          tryCatch({
            ...
            if (ncol(file_entry$methylation_data_train.final) > 1) {
              ...
              train_predicted_ages <- predict(lasso_fit, newx=  file_entry$methylation_data_train.final, s = cv_fit$lambda.min)
              test_predicted_ages <- predict(lasso_fit, newx = file_entry$methylation_data_test.final, s = cv_fit$lambda.min)
        ...
        if (!is.null(file_entry$final_clock) && !is.null(file_entry$final_validation_metadata)) {
          ...
          if (ncol(final_validation_methylation_data) > 0) {
            final_val_predicted_ages <- predict(file_entry$final_clock$glmnet_fit, newx = final_validation_methylation_data )
